...
/**
 * 累积性解码器 A composite {@link ProtocolDecoder} that demultiplexes incoming
 * {@link IoBuffer} decoding requests into an appropriate {@link MessageDecoder}
 * .
 * 
 ...
 * {@link MessageDecoder}s and calls
 * {@link MessageDecoder#decodable(IoSession, IoBuffer)}. Initially, all
 * registered {@link MessageDecoder}s are candidates.</li>
 * <li>If {@link MessageDecoderResult#NOT_OK} is returned, it is removed from
 * the candidate list.</li>
 * <li>If {@link MessageDecoderResult#NEED_DATA} is returned, it is retained in
 * the candidate list, and its
 * {@link MessageDecoder#decodable(IoSession, IoBuffer)} will be invoked again
 * when more data is received.</li>
 * <li>If {@link MessageDecoderResult#OK} is returned,
 * {@link DemuxingProtocolDecoder} found the right {@link MessageDecoder}.</li>
 ...
 * </ol>
 * 
 * Please note that any change of position and limit of the specified
 * {@link IoBuffer} in {@link MessageDecoder#decodable(IoSession, IoBuffer)}
 * will be reverted back to its original value.
 * <p>
 * Once a {@link MessageDecoder} is selected, {@link DemuxingProtocolDecoder}
 * calls
 * {@link MessageDecoder#decode(IoSession, IoBuffer, ProtocolDecoderOutput)}
 * continuously reading its return value:
 * <ul>
 * <li>{@link MessageDecoderResult#NOT_OK} - protocol violation;
 * {@link ProtocolDecoderException} is raised automatically.</li>
 * <li>{@link MessageDecoderResult#NEED_DATA} - needs more data to read the
 * whole message;
 * {@link MessageDecoder#decode(IoSession, IoBuffer, ProtocolDecoderOutput)}
 * will be invoked again when more data is received.</li>
 * <li>{@link MessageDecoderResult#OK} - successfully decoded a message; the
 * candidate list will be reset and the selection process will start over.</li>
 * </ul>
 * 
