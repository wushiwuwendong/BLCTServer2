...
/**
 * 这个类继承自 {@link CumulativeProtocolDecoder} 类，用于读取数据最前端的 1、2、4 个字节
 * 表示后面的数据长度的数据。譬如：一个段 数据的前两个字节表示后面的真实数据的长 度，那么你就可以用这个方法进行解码。
 * 
...
public class PrefixedStringDecoder extends CumulativeProtocolDecoder {
	...
	/**
	 * Sets the maximum allowed value specified as data length in the incoming
	 * data
	 * <p>
	 ...
	 * {@link PrefixedStringDecoder#DEFAULT_MAX_DATA_LENGTH}.
	 * </p>
	 * 
	 * @param maxDataLength
	 *            maximum allowed value specified as data length in the incoming
	 *            data
	 */
