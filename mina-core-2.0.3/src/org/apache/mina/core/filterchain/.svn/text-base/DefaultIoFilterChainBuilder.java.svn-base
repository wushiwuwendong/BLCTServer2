 ...
 * 默认实现的IoFilterChainBuilderñ这是大多数情况下有用
 * 。DefaultIoFilterChainBuilder已经与oFilter相同的接口，它包含了IoFilter列表，您可以修改。
 * 哪些被添加到这个建筑工地oFilter将被追加到IoFilterChain中时buildFilterChain（IoFilterChain中）被调用。
 * <p>
 ...
 * {@link IoSession}s. 然而，相同的接口并不意味着它在一个完全与IoFilterChain中同样方式行事。
 * DefaultIoFilterChainBuilder不管理在所有的IoFilter生命周期
 * ，现有IoSession不会得到在这uilder影响的变化。IoFilterChainBuilder只会影响新创建的IoSession。
 * 
 * <pre>
 * IoAcceptor acceptor = ...;
