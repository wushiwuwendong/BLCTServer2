...
/**
 * 为会话提供所需要的数据结构 默认IoSessionDataStructureFactory实现，创建一个新的IoSessionAttributeMap
 * HashMap的实例和新的同步每IoSession的ConcurrentLinkedQueue实例
 * 。它提供的写请求队列内部是一个初始大小为16的循环队列，并且在插入队列尾部和从队列头部取数据时都必须满足互斥同步。
 * 
